...
function getFullPhone() {
  const code = document.getElementById("modal-country-code").value;
  const local = document.getElementById("modal-phone").value.trim().replace(/[^\d]/g, "").replace(/^0+/, "");
...
async function confirmBooking() {
  ...
  closeBookingModal();
  await selectDate(currentDate);
  const landmarkPart = CONFIG.landmark ? ` (${CONFIG.landmark})` : "";
  document.getElementById("summary").textContent =
    `Thank you for booking, ${name.split(" ")[0]}! See you at ${CONFIG.address}${landmarkPart}. Save your phone number to check "My bookings" later - we'll verify your receipt shortly.`;
}
  ...
  if (nextBtn) {
    nextBtn.addEventListener("click", () => {
      ...
      if (!firstName || !lastName) {
        document.getElementById("modal-error").textContent = "Please enter your first and last name.";
        return;
      }
      if (!phone) {
        document.getElementById("modal-error").textContent = "Please enter your phone number.";
        return;
      }
